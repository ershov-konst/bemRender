...
var
   ...
   async = require('async'),
   mapModules = require('./mapModules');

/**
 *
 * @param config
 * @param config.express - express application
 * @param config.rootPath - root path of application
 * @param config.cache - caching template functions
 * @param config.blocksPath - path with blocks
 * @param config.publicPath - path for public access
 * @param config.nameRule - rule for naming blocks/elements/modifiers
 * @return {Function}
 */
module.exports = function(config){
   ...
   if (!config.rootPath){
      throw new Error("root path of application is not defined for bemRender");
   }

   config.debug = config.debug || false;
   config.cache = config.cache || false;
   config.blocksPath = config.blocksPath || path.join(config.rootPath, "blocks");
   config.publicPath = config.publicPath || path.join(config.rootPath, "public");
   config.resourcePath = path.join(config.rootPath, ".source");
   ...
   try{
      mkdipr.sync(config.resourcePath);
      config.express.use("/pagesource", express.static(config.resourcePath));
      var
         paths = mapModules.generateMap(config.publicPath),
         confTemplate = fs.readFileSync(path.join(__dirname, "templates/requirejsConf.dot")),
         text = doT.template(confTemplate)({paths : JSON.stringify(paths)});

      fs.writeFileSync(path.join(config.resourcePath, "main.js"), text);
   }
   catch (e){
      throw e;
   }
   ...
   bemRender.prototype.prepareJS = function(handler){
      ...
      if (config.debug){
         ...
         libsArray.push("/lib/ext/module.js");
         ...
         libsArray.push("/pagesource/libs.js");
         try{
            var temp = [];
      ...
      async.parallel(asyncArray, function(err, results){
         if (!err){
            fs.readFile(path.join(__dirname, "templates/bemStart.dot"), "utf8", function(err, res){
               if (!err){
                  var dotFunc = doT.template(res);
                  handler(null, dotFunc({
                     preparedJS  : preparedJS ? path.join("/pagesource", preparedJS) : false, //путь к пакету из модулей блоков и клиентских модулей
      ...
      less.render(indexLess.join("\n"), function(err, css){
         if (err){
            ...
            self.savePackage(cssPath, css, function(err){
               if (!err){
                  handler(null, '<link rel="stylesheet" href="'+ path.join("/", "pagesource", cssPath) +'"/>');
               }
               else{
                  handler(err);
               }
   ...
   bemRender.prototype.savePackage = function(name, str, handler){
      var
         filePath = path.join(config.resourcePath, name),
