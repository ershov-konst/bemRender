   ...
   bemRender.prototype.prepareTemplate = function(json, parent){
      ...
      for (var i in json){
         if (json.hasOwnProperty(i)){
            if (i == "content" && json[i] instanceof Array && !isMultiple){
               var content = [];
      ...
      if (isMultiple && "content" in json){
         if (json["content"] instanceof Array){
            html = html.replace(/it\.content/g,"it.content[i]");
            html = ["{{for (var i=0, l=it.content.length; i < l; i++){}}",html,"{{}}}"].join("");
         }
         else{
            html = html.replace(/\}\}/g, "}|}");
            /*var tplRes = doT.template(html)(json);*/
         }
      }
