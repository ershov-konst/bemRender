...
module.exports = function(config){
   ...
   var bemRender = function(jsonPath, options, fn){
      ...
      if (config.cache && jsonPath in cache && typeof cache[jsonPath] == "function"){
         ...
         fs.readFile(jsonPath, 'utf8', function(err, str){
            ...
            //TODO : options.getElementName(blockName, elementName)
            //TODO : options.getMod(blockName, mod, value)
   ...
   bemRender.prototype.prepareTemplate = function(json, parent){
      ...
      for (var i in json){
         ...
         serviceVars["getBlockName"] = function(n){
            return self.getCssName(isElement, parent, name + "__" + n);
         };
      ...
      for (var v in serviceVars){
         if (serviceVars.hasOwnProperty(v)){
            html = html.replace(new RegExp("\\{\\{.+it\\.(" + v + ")(.*?)\\}\\}", "g"), function(str, code, args){
               var res = serviceVars[code];
               if (json["multiple"] && code == "content" && typeof res == "string"){
