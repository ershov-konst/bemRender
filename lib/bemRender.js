...
module.exports = function(config){
    ...
    * @param handler
    */
   function savePackage(baseName, name, str, handler){
   ...
   return function(jsonPath, options, fn){
      if (jsonPath in cache && typeof [jsonPath] == "function"){
         var
         ...
         fs.readFile(jsonPath, 'utf8', function(err, str){
            ...
            prepareCSS(blocksHash, addCSS, function(e, css){
               if (e) fn(e);
               savePackage(baseName, "all.css", css, function(err){
                  if (err)
                     fn(err);
                  else
                     prepareJS(blocksHash, addJS, function(js){
                        if (!js.length) fn(null, result);
                        savePackage(baseName, "all.js", js, function(err){
                           if (err) fn(err);
                           try{
