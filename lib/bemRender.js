...
module.exports = function(config){
   ...
   bemRender.prototype.prepareJS = function(handler){
      ...
      try{
         indexJS.push(fs.readFileSync(path.join(__dirname, "/ext/jquery.js")));
      ...
      indexJS.push("$br.config.setConfig(" + JSON.stringify(this.bemJson) + ");");
      for (var i in blocks){
         if (blocks.hasOwnProperty(i)){
            ...
            try{
               ...
               js = js.replace(/;\s*$/, "");
               indexJS.push("$br.tech.set('"+ blocks[i]["css"] +"', 'js', " + js + ");");
               if (blocks[i]["html"]){
                  indexJS.push("$br.tech.set('"+ blocks[i]["css"] +"', 'html', \"" + encodeURIComponent(blocks[i]["html"]) + "\");");
               }
            }
            catch(e){}
         }
